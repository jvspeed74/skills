...
**Environment note:** The four paths above are hosted-sandbox (Claude.ai Skills) conventions,
where they are correct and must be used as written. When this skill runs under Claude Code as
part of the `mcq-probe` plugin bundle, resolve all four relative to `${CLAUDE_PLUGIN_ROOT}`
instead:
