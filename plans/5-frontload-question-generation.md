...
| A-9 | **FM-C-7's threshold was set too low, and the prompt figure was stale.** Measured with `token-counter` (the tool used throughout this program — a character-count estimate had inflated an earlier draft of this row): prompts total **48,399 tokens** pre-#9 — MAT 18,479 / ORD 12,420 / MSQ 8,894 / MCQ 8,606 — not "~47k" and not the 58.9k a char-based estimate suggested. Atoms add ~250–900 tokens per trial, so a 10-trial batch measures **~7.5k–22.5k** — breaching the original ~15k flag on Matching-heavy batches. Threshold raised to 25k; the real consequence is an input to **#4's** window-size decision, not a blocker here. | #9 |
| A-11 | **#9's baking grows the prompts by +8,645 tokens (+17.9%), 48,399 → 57,044.** Concentrated in the per-field forcing citations and emission gates — i.e. in the FM-C-1 guard itself, so trimming it means trimming the guard. **Accepted.** Three reasons: the cost is paid once per session, not per trial; it buys the mechanism that makes the every-wrong-answer-viable invariant auditable at generation time, which is the deliverable; and **#6 is about to make prompt loading conditional on the I5/I6 gates**, so this grows a number that is shortly to stop being paid unconditionally — an MCQ/MSQ-only session will load 19,271 tokens, not 57,044. Revisit only if #6's gating slips. | orchestrator |
...
| A-12 | **REQ-C-008's three checks are withdrawn and replaced.** Scenario freshness is already gated in all four `<internal-validation>` checklists (redundant); domain-anchor variety contradicts `<domain-anchoring>`, which binds every trial to intake I3's single domain (incoherent); correct-answer-position clustering is statistically inert at realistic N — with four types drawn uniformly, E[MCQ slots] is 0.75 at N=3 and 2.25 at N=9, and P(≥3 MCQ slots) is 1.6% at N=3, below the threshold at which clustering separates from noise. Replaced by: (a) `select_answer_position.py`, drawn in Pass 1 alongside type and axis, fixing positional bias by construction rather than rejection and matching the existing externalized-randomization precedent; and (b) G6 as an atom-completeness sweep over the finalized batch — the check FM-C-3 and #9's FM-9-3 both promised and REQ-C-008 never carried. REQ-C-009's regenerate-only-the-offending-trial rule and FM-C-6's 3-attempt cap carry over unchanged. | #10 |
...
| ~~REQ-C-008~~ | ~~A consistency pass runs over the finalized batch: no repeated scenario, no clustered correct-answer position, domain-anchor variety~~ — **withdrawn by A-12.** Replaced by REQ-C-016 and REQ-C-017 | #10 |
| REQ-C-016 | `SCRIPT_POSITION` (`select_answer_position.py`) is drawn in Pass 1 for every `mcq` slot and fixes that slot's `key`; Pass 2 constructs around it. MCQ only — Ordering and Matching have no assignable position, MSQ is a follow-up  [A-12] | #10 |
| REQ-C-017 | G6 sweeps the finalized batch and verifies every slot's `explanation` block against its type's coverage rule; an incomplete slot is regenerated, a complete slot untouched  [A-12] | #10 |
