...
Run all twelve. Report every finding; never stop at the first.

| # | Finding when |
|---|---|
| 1 | `## Open Questions` has any row |
| 2 | An `INV-XX` appears in no Design Decision and no Failure Mode |
| 3 | An `FR-XX` or `NFR-XX` appears in no Design Decision's `Satisfies` |
| 4 | A Files Touched row's `Why` names no `DD-XX`, `FR-XX`, `NFR-XX` or `INV-XX` |
...
- `PLAN_AUDIT_OK: <plan filename> — 12 checks, clean`
